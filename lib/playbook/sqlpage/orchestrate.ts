...
import { strategies } from "../../universal/resource.ts";
import {
  isRouteSupplier,
  PageRoute,
  RoutesBuilder,
...
export async function* sqlPageFiles(
  ...
  for (const c of contribSPFs) {
    ...
    const resources = Array.from(contribs.prepared());
    yield jsonSPF(
      `spry.d/auto/contribute/${c.identity}.auto.json`,
      safeJsonStringify({
        origin: { blockBases: contribs.blockBases, issues: contribs.issues },
        resources,
      }, 2),
    ...
    for await (
      const r of strategies(
        resources.map((r) => {
          const { ppiq } = r.origin;
          return {
            ...r.provenance,
            mimeType: ppiq.getTextFlag("mime") ?? r.provenance.mimeType,
            spfPath: r.destPath,
          };
        }),
      )
